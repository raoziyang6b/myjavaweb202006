...
public class BookInfoServiceImpl implements IBookInfoService {
    ...
    public BookInfo adminFindById(int bookId) throws BSException {
        ...
        if(bookInfo == null){
            throw new BSException("您搜索的商品不存在!");
        }
        return bookInfo;
    }
    ...
    @Override
    public List<BookInfo> findAllAllBylook() {
        List<BookInfo> list =   bookInfoMapper.selectAll();

        List<BookInfo> BookInfoList = list.stream().sorted(Comparator.comparing(BookInfo::getLookMount).reversed()).collect(Collectors.toList());
        if(BookInfoList!=null){
            BookInfoList =  BookInfoList.subList(0,3);
        }
        return BookInfoList;
    }
}
